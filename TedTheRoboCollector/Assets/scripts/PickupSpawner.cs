...
public class PickupSpawner : MonoBehaviour
{
	...
	Timer spawnTimer;
    const int MaxNumPickups = 20; // 20
    const int TotalPickups = 50; // 50
    int spawnedCount = 0;
	...
	void HandleSpawnTimerFinishedEvent()
    {
        ...
        if (GameObject.FindGameObjectsWithTag("Pickup").Length < MaxNumPickups && spawnedCount < TotalPickups)
        {
			SpawnPickup();
            spawnedCount++;
		}
    ...
    {		
		...
		if (Physics2D.OverlapArea(min, max) == null)
        {
            ...
            pickup.name = spawnedCount.ToString();
