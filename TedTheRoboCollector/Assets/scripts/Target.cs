...
public class Target : IComparable
{
    ...
    public int CompareTo(object obj)
    {
        ...
        if (otherTarget != null)
        {
            if (this.distance > otherTarget.distance) // larger distances should be earlier in list
            {
                return -1;
            }
            else if (Mathf.Abs(this.distance - otherTarget.distance) < 0.005) // testing floats for equality is tricky
            {
                return 0;
            }
            else
